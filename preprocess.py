...
def load_data(data_path: str, years: list, sample_size: int) -> pd.DataFrame:
    ...
    if len(df_concat) > sample_size:
        ...
        raise ValueError(
            "No data loaded after sampling. Check sample size and data paths.")
...
def process_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[TARGET_COLUMN] = (df[DROPOFF_DT_COL] -
...
def main(train_size: int):
    ...
    print(
        f"Calculated Holdout Size (30%): {holdout_size} (from {HOLDOUT_YEARS})")
    print(
        f" -> This will be split into: {holdout_size // 2} (Val) and {holdout_size - (holdout_size // 2)} (Test)")
    print("-" * 50)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Preprocess NYC Taxi data with temporal split.")
